...
namespace TerumoMIS.CoreLibrary.Algorithm
{
    ...
    /// 快速排序
    /// </summary>
    internal static partial class QuickSortPlus
    {
        /// <summary>
        /// 排序器
        /// </summary>
        ...
        public static TValueType[] GetSort<TValueType>(TValueType[] values, Func<TValueType, TValueType, int> comparer)
        {
            if (values.Length() != 0)
            {
                SorterStruct<TValueType> sorter = new SorterStruct<TValueType> { Array = values.Copy(), Comparer = comparer };
                if (values.Length > 1)
                {
                    if (comparer == null) log.Error.Throw(log.exceptionType.Null);
        ...
        public static void Sort<TValueType>(TValueType[] values, Func<TValueType, TValueType, int> comparer, int startIndex, int count)
        {
            array.range range = new array.range(values.Length(), startIndex, count);
            if (range.GetCount > 1)
            {
                if (comparer == null) log.Error.Throw(log.exceptionType.Null);
                new SorterStruct<TValueType> { Array = values, Comparer = comparer }.Sort(range.SkipCount, range.EndIndex - 1);
            }
        }
        /// <summary>
        /// 数组范围排序
        ...
        /// <param name="count">排序数据数量</param>
        /// <returns>排序后的新数组</returns>
        public static TValueType[] GetSort<TValueType>(TValueType[] values, Func<TValueType, TValueType, int> comparer, int startIndex, int count)
        {
            array.range range = new array.range(values.Length(), startIndex, count);
        ...
        private struct RangeSorterStruct<TValueType>
        {
            ...
            public void Sort(int startIndex, int endIndex)
            {
                do
                {
                    ...
                    int leftIndex = startIndex, rightIndex = endIndex;
                    TValueType value = Array[average];
                    if (Comparer(leftValue, value) <= 0)
                    {
                        if (Comparer(value, rightValue) > 0)
                        {
            ...
            (TValueType[] values, Func<TValueType, TValueType, int> comparer, int skipCount, int getCount)
        {
            ...
            if ((getCount = range.GetCount) != 0)
            {
                ...
                return subArray<TValueType>.Unsafe(values, range.SkipCount, getCount);
            }
            ...
            (TValueType[] values, Func<TValueType, TValueType, int> comparer, int skipCount, int getCount)
        {
            ...
            if ((getCount = range.GetCount) != 0)
            {
                ...
                sorter.Sort(0, values.Length - 1);
                return subArray<TValueType>.Unsafe(sorter.Array, range.SkipCount, getCount);
            }
            return default(subArray<TValueType>);
        }
        /// <summary>
        ...
        /// <returns>排序后的数据</returns>
        public static subArray<TValueType> RangeSort<TValueType>
            (TValueType[] values, int startIndex, int count, Func<TValueType, TValueType, int> comparer, int skipCount, int getCount)
        {
            array.range range = new array.range(values.Length(), startIndex, count);
            if ((count = range.GetCount) != 0)
            {
                ...
                if ((getCount = getRange.GetCount) != 0)
                {
                    ...
                    return subArray<TValueType>.Unsafe(values, skipCount, getCount);
                }
            }
            return default(subArray<TValueType>);
        }
        ...
        /// <param name="count">排序数据数量</param>
        /// <returns>排序后的数据</returns>
        public static subArray<TValueType> GetTop<TValueType>(TValueType[] values, Func<TValueType, TValueType, int> comparer, int count)
        {
            if (values == null) return default(subArray<TValueType>);
            if (comparer == null) log.Error.Throw(log.exceptionType.Null);
            if (count > 0)
            {
                ...
                return subArray<TValueType>.Unsafe(values, 0, values.Length);
            }
            return default(subArray<TValueType>);
        }
        /// <summary>
        ...
        /// <param name="count">排序数据数量</param>
        /// <returns>排序后的数据</returns>
        public static subArray<TValueType> Top<TValueType>(TValueType[] values, Func<TValueType, TValueType, int> comparer, int count)
        {
            if (values == null) return default(subArray<TValueType>);
            if (comparer == null) log.Error.Throw(log.exceptionType.Null);
            if (count > 0)
        ...
        /// <param name="count">排序数据数量</param>
        /// <returns>排序后的数据</returns>
        private static subArray<TValueType> GetTopBase<TValueType>(TValueType[] values, Func<TValueType, TValueType, int> comparer, int count)
        {
            uint sqrtMod;
            var length = Math.Min(Math.Max(count << 2, count + (int)number.sqrt((uint)values.Length, out sqrtMod)), values.Length);
            ...
            var sort = new RangeSorterStruct<TValueType> { Array = newValues, Comparer = comparer, SkipCount = count - 1, GetEndIndex = count - 1 };
            sort.Sort(0, --length);
            for (TValueType maxValue = newValues[sort.GetEndIndex]; readIndex != 0; )
            {
            ...
            return subArray<TValueType>.Unsafe(newValues, 0, count);
        }
        /// <summary>
        /// 排序去除Top N
        /// </summary>
        ...
        /// <param name="count">排序数据数量</param>
        /// <returns>排序后的数据</returns>
        private static TValueType[] GetRemoveTopBase<TValueType>(TValueType[] values, Func<TValueType, TValueType, int> comparer, int count)
        {
