...
//  All rights reserved
//	Input Item: PhysicalPlus
//	PC Name:    PC0525
//	Name Space: TerumoMIS.CoreLibrary.MemoryDataBase
//	File Name:  PhysicalPlus
//	User name:  C1400008
//	Location Time: 2015/7/16 13:07:07
...
namespace TerumoMIS.CoreLibrary.MemoryDataBase
{
    ...
    /// 内存数据库物理层
    /// </summary>
    internal sealed class PhysicalPlus:IDisposable
    {
        /// <summary>
        /// 文件读取器
        /// </summary>
        ...
        private void dataError()
        {
            if (Interlocked.CompareExchange(ref currentLock, 1, 0) == 0)
            {
                LastException = dataException;
                currentLock = 2;
                fastCSharp.threading.task.Tiny.Add(Dispose);
            }
        }
        /// <summary>
        /// 检测当前操作
                        ...
                        fixed (byte* currentBufferFixed = currentBuffer)
                        {
                            ...
                            else
                            {
                                fixed (byte* dataFixed = compressData.array)
                                {
                                    ...
                                    fastCSharp.unsafer.memory.Copy(dataFixed + sizeof(int), currentBufferFixed + sizeof(int), compressData.Count);
        ...
        private unsafe void flush(byte* bufferFixed)
        {
            ...
            else
            {
                fixed (byte* dataFixed = compressData.array)
                {
                    int count = compressData.Count + sizeof(int);
                    fastCSharp.unsafer.memory.Copy(dataFixed + sizeof(int), bufferFixed + sizeof(int), compressData.Count);
                    *(int*)dataFixed = -count;
